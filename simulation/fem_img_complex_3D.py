...
'''
Experiment - 3D Heterogeneous Version
'''
...
for i, id in enumerate(common_ids[:num_samples]):
    ...
    def sigma(u):
        ...
        return 2 * mu * eps + lam * ufl.tr(eps) * ufl.Identity(3)  # 3D identity
    ...
    # Interpolate stiffness parameters
    mu_real = scipy.interpolate.griddata(coordinates[:,:3], mu_vals.real, 
                                        (grid_x, grid_y, grid_z), method='linear', fill_value=0)
    lam_real = scipy.interpolate.griddata(coordinates[:,:3], lam_vals.real, 
                                         (grid_x, grid_y, grid_z), method='linear', fill_value=0)
    mu_imag = scipy.interpolate.griddata(coordinates[:,:3], mu_vals.imag, 
                                        (grid_x, grid_y, grid_z), method='linear', fill_value=0)
    lam_imag = scipy.interpolate.griddata(coordinates[:,:3], lam_vals.imag, 
                                         (grid_x, grid_y, grid_z), method='linear', fill_value=0)
    ...
    stiff_map = np.concatenate(
        (np.expand_dims(mu_real, 0), np.expand_dims(lam_real, 0), 
         np.expand_dims(mu_imag, 0), np.expand_dims(lam_imag, 0)), 0)
...
with open('data_general_heter_3D.pkl', 'wb') as handle:
    pickle.dump(all_data, handle)
print('All 3D heterogeneous samples generated and saved to data_general_heter_3D.pkl') 
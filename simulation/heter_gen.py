...
for id in range(currentID, currentID+num_samples):
    ...
    class fun_gen():
        def __init__(self, image_file_path, mask_image_file_path, size_scale, mu_min=2.0, mu_max=4):
            ...
            self.E_coor, E_vals = self._load_E_from_image(image_file_path, self.liver_mask_vals, tumor_mask_vals, mu_min, 2*np.random.rand()+3)
    ...
    # Example usage
    funcls = fun_gen('/taiga/illinois/eng/cee/meidani/Vincent/MRE/color/field_{}.png'.format(id), 
        '/taiga/illinois/eng/cee/meidani/Vincent/MRE/liver_masks/mask_{}.pkl'.format(id), size_scale)
